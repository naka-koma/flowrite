...
6. **マージ検知後の自動対応:** 購読中のPRがマージされたイベントを検知したら、ユーザーへの確認なしで以下を続けて実行する
   ```bash
   git checkout main
   git pull
   git branch -d <branch>
   ...
   - webhookイベントが届かない場合（セッション終了等）に備え、ユーザーから「マージしました」と伝えられた場合も同様にクリーンアップ・デプロイまで自動で行う

#### B. Plan先行パターン（計画してからIssueを立てる）

1. **計画:** ユーザーと一緒に設計・実装計画を練る
2. **Issue作成:** `gh issue create` で計画内容をIssueに記録する
3. **以降はAと同じ:** ブランチ作成→実装→PR（`Closes #<number>`付き）→PR購読→マージ検知後の自動クリーンアップ・デプロイ
