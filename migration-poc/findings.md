...
**注記（検証時点）:** 以下の検証でCloudflare（Workers/D1）を使っている箇所があるが、これは「アカウント登録不要でローカル完結できて検証が手軽だった」という理由だけで選んでいる。検証したSQLは標準的なもの（`GROUP BY`/`CASE WHEN`等）で、PostgreSQLなど他のSQL DBでもほぼそのまま通用する内容である。#217の「特定サービスへのロックインを避ける」方針の通り、D1を採用候補として推す意図はない（後述の通り、最終的にもD1は採用していない）。

## 0. 移行先の最終決定

Phase 1の検証・比較を踏まえ、以下で確定した（2026-08-17）。**具体的なサービス名の決定はこのファイルにのみ記録し、#217本体には書かない方針（一般的な計画記録として維持するため）。**

| レイヤー | 決定 |
|---|---|
| ホスティング（フロント・API） | Cloudflare Workers + Pages |
| データストア | Neon（サーバーレスPostgres） |
| 利用プラン | 無料枠のみ（クレジットカード登録なし）でまず開始する |

### 決定理由

- **Fly.io / Renderは無料枠が実質使えないため候補から除外した。** Fly.ioは2024年に無料枠を廃止済み（新規は2時間/7日のトライアルのみ、以降はカード必須で実質月$5〜）。Renderの無料PostgreSQLは作成から30日で失効し、猶予14日後にデータごと削除される（実運用データを預けるには不適）
- 無料かつ実運用データを預けられる現実的な候補として、Cloudflare（Workers/Pages/D1）とNeon（サーバーレスPostgres、無期限無料・カード不要）が残った
- **Cloudflare D1ではなくNeonを選んだ。** 理由はPostgresという標準技術であることに加え、Neonのドライバ（`@neondatabase/serverless`）がHTTP経由でアクセスでき、Workers専用ではなく通常のNode.js等からも同じコードで使えるため。D1はWorkersの専用binding APIでしか動かせずロックインが強い
- APIのルーティング層は、Workers/Node/Deno/Bunいずれでも動くフレームワーク（例: Hono）を使うことで、将来Cloudflareを離れる場合の移植コストをさらに下げる方針とする

### Neon無料枠の制約と対応

- 上限は0.5GBストレージ・月100CU時間（コンピュート）。`raw_data`相当のテーブルの容量試算（1行あたり1KB換算で5万行=50MB程度）では、当面（数年〜十年単位）は十分収まる見込み
- **カード未登録のため、上限超過時は自動課金ではなく「サスペンド」状態になる。** サスペンド中は書き込み（INSERT/UPDATE/DELETE）が失敗するが、データが消えることはない。不要データの削除、または有料プランへのアップグレードで復帰する
- Phase 2以降で、DB使用量を監視できる仕組み（設定画面での使用量表示等）の導入を検討する
