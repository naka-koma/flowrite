...
- 大項目単位のリネーム・削除に対応する（設定画面のカテゴリ設定から操作できる）。中項目単位のリネーム・削除には対応しない
  - リネーム: 該当大項目を持つ全ての行のcategoryを一括更新する。対応する`budgets`シートの予算行も追従して更新する
  - 削除: 該当大項目を持つ全ての行を削除する。対応する`budgets`シートの予算行も削除する
  - **`raw_data`（過去の取引データ）は更新しない**。リネーム・削除後も過去の取引は旧カテゴリー名のまま残り、実績集計上は新旧で別カテゴリとして扱われる（意図的な仕様）
...
| 列 | カラム名 | 型 | 説明 |
|---|---|---|---|
| A | category | string | 大項目（`categories`シートに存在するものに限る） |
...
- `categories`シートに存在しない大項目に対して予算を追加することはできない
