...
- 削除・リネームには対応しない（本シートを直接編集する運用は想定しない）
...
| 列 | カラム名 | 型 | 説明 |
|---|---|---|---|
| A | key | string | 属性の項目名（例: `ワークスタイル`） |
| B | value | string | 属性の内容（例: `在宅リモートワーク中心`） |

### 注意
- 1行目はヘッダー行とする
- 1キーにつき1行。同じキーで追加する場合は既存行の値を上書きする（`handleUpsertAiAttribute`）
- AIアドバイス取得時、登録されているすべてのペアがプロンプトの先頭セクションに結合される
