...
| プロパティ | 型 | 必須 | 説明 |
|---|---|---|---|
| csv | string | 必須 | Base64エンコードされたCSVデータ（Shift-JIS） |
| overwriteCategory | boolean | - | 既存id行のcategory/subcategory/memoをCSV側の値で上書きするか。省略時は`true` |
...
- `overwriteCategory`が`true`（デフォルト）の場合、既存idの行はcategory/subcategory/memoのうちCSV側の値が空でない項目のみ上書きし、`updatedAt`を更新する（空の場合は既存値を保持する）。date/amount/institutionなどその他の列は変更しない
- `overwriteCategory`が`false`の場合、既存idの行のcategory/subcategory/memoは一切変更しない（新規行の追加は通常通り行う）
- 既存idの行の`categoryLocked`が`true`の場合、`overwriteCategory`の設定に関わらずcategory/subcategory/memoを一切上書きしない
...
## `handleGetAiFocusPoints(body)`

AIアドバイスウィザードの②ステップ。対象期間のデータ・予算対比・ユーザー属性・過去の気づき（メモリ）・関心テーマを踏まえ、Geminiに「気になる点」の候補を2〜4件提示させる。ユーザーはこの中から1件選んで`handleStartAiChat`の`agendaTopic`に渡し、深掘り対話を始める。
...
- プロンプトには`settings`シートの`agendaTopics`（ユーザーが関心を持ちやすいテーマの参考情報）と、`ai_memory`シートの`insight`型メモリ（過去の気づき・傾向）を注入する
- Gemini APIキー未設定時は`{ "success": false, "error": "GEMINI_API_KEY is not set in script properties" }`を返す

---

## `handleStartAiChat(body)`

AIアドバイスウィザードの③ステップ。②で選んだ「気になる点」を`agendaTopic`として深掘り対話を開始し、Gemini APIの構造化出力（`responseSchema`）を使って`{ai_message, quick_replies, is_final, todo_actions}`を1回のリクエストで取得する。

**引数**
```js
{
  "agendaTopic": "外食費が先月より増えています: 今月は外食費が先月より増加傾向です。理由を深掘りしましょう。",
  "summaryParams": { "unit": "month", "year": 2025, "month": 12 }
}
```
...
| プロパティ | 型 | 必須 | 説明 |
|---|---|---|---|
| agendaTopic | string | 必須 | `handleGetAiFocusPoints`が返した`focusPoints`のうちユーザーが選んだ1件（`title`+`context`を結合した文字列） |
| summaryParams | `SummaryParams` | 必須 | 分析対象データの期間（`handleSummary`と同じ形式） |
...
プロンプトは以下のセクションを上から順に結合して生成する（属性情報・気づき・分析対象データ・予算対比はそれぞれ内容が無い場合は省略する）。
...
## `handleGetAiCategorySuggestions(body)`

指定した年月の取引に対し、Gemini APIでカテゴリ分類を提案する。`raw_data`は変更しない（提案の返却のみ）。
...
```js
{
  ...
  "month": 12,
  "scope": "uncategorized",
  "categoryFilter": [{ "category": "その他", "subcategory": "雑費" }],
  "institutionKeyword": "楽天",
  "contentKeyword": "Amazon",
  "amountMin": 1000,
  "amountMax": 5000
}
```
...
| year | number | 必須 | 対象年 |
| month | number | 必須 | 対象月 |
| scope | `"uncategorized" \| "all"` | 必須 | `"uncategorized"`は大項目が空の行のみ、`"all"`は`categoryLocked`がtrueの行を除く対象月の全行を提案対象にする |
| categoryFilter | `{category, subcategory}[]` | - | 指定した(大項目, 中項目)ペアに一致する行のみ対象にする。省略・空配列ならフィルタなし |
| institutionKeyword | string | - | 保有金融機関名の部分一致フィルタ。省略・空文字ならフィルタなし |
| contentKeyword | string | - | 内容（店舗名）の部分一致フィルタ。省略・空文字ならフィルタなし |
| amountMin / amountMax | number | - | 支出額（絶対値）の範囲フィルタ。いずれか一方でも指定した場合、収入（amountが0以上の行）は常に対象外になる |
  ...
  "suggestions": [
    {
      ...
      "suggestedSubcategory": "スーパー",
      "isNewCategory": false,
      "reason": "内容から食料品の購入と判断"
    }
  ],
  "targetCount": 1
}
...
- プロンプトには`ai_memory`シートの`categoryPattern`型メモリ（ユーザーが「記憶する」で保存した分類パターン）を参考例として注入する。完全一致するパターンがあってもGemini呼び出しはスキップしない（あくまで参考情報として扱う）
- 対象0件の場合は`{ "success": true, "suggestions": [], "targetCount": 0 }`を返す
- Gemini APIキー未設定時は`{ "success": false, "error": "GEMINI_API_KEY is not set in script properties" }`を返す
- 各バッチのプロンプト・応答は`ai_log`シートに記録される

---

## `handleApplyAiCategorySuggestions(body)`

`handleGetAiCategorySuggestions`で得られた提案のうち、ユーザーが選択したものだけを`raw_data`に一括反映する。

**引数**
```js
{
  "suggestions": [
    { "id": "<取引ID>", "category": "食費", "subcategory": "スーパー" }
  ]
}
```
...
**戻り値**
```js
{ "success": true, "applied": 1, "notFound": 0 }
```

**注意**
- 反映対象は大項目・中項目のみ（メモは変更しない）。`updatedAt`を更新し、`categoryLocked`を`true`にする（以後のCSV再取込で上書きされなくなる）
- 存在しないidは`notFound`としてカウントし、処理は継続する
- `suggestions`が空の場合は`{ "success": false, "error": "suggestions is required" }`を返す
...
登録されているAIメモリ（気づき・傾向、または分類パターン）の一覧を返す。引数なし。

**戻り値**
```js
{
  "memories": [
    { "id": "xxxxxxxx-xxxx-...", "type": "insight", "content": "外食費を気にしている", "category": "", "subcategory": "", "createdAt": "2026-07-01T12:00:00.000Z" },
    { "id": "yyyyyyyy-yyyy-...", "type": "categoryPattern", "content": "スターバックス", "category": "食費", "subcategory": "外食", "createdAt": "2026-07-02T09:00:00.000Z" }
  ]
}
```

**注意**
- `type`は`"insight"`（気づき・傾向）または`"categoryPattern"`（分類パターン）
- `category`/`subcategory`は`categoryPattern`のみ使用する（`insight`では空文字）

---

## `handleAddAiMemory(body)`

メモリを1件新規追加する。idと`createdAt`はGAS側で生成する。常にユーザーの明示操作（AIアドバイスの「覚えておく」ボタン、AI分類提案の「記憶する」チェック）からのみ呼ばれ、AIが自動生成することはない。
...
| プロパティ | 型 | 必須 | 説明 |
|---|---|---|---|
| type | `"insight" \| "categoryPattern"` | 必須 | メモリの種類 |
| content | string | 必須 | insight: 気づきの本文。categoryPattern: 元になった取引の内容・金融機関などの参考テキスト |
| category | string | typeが`categoryPattern`のとき必須 | 学習した大項目 |
| subcategory | string | typeが`categoryPattern`のとき必須 | 学習した中項目 |

**戻り値**
```js
{
  "success": true,
  "memory": { "id": "xxxxxxxx-xxxx-...", "type": "insight", "content": "外食費を気にしている", "category": "", "subcategory": "", "createdAt": "2026-07-01T12:00:00.000Z" }
}
```

**注意**
- `type`が`"insight"`/`"categoryPattern"`のいずれでもない場合はエラーを返す
- `content`が空文字の場合はエラーを返す
- `type`が`"categoryPattern"`で`category`/`subcategory`のいずれかが空文字の場合はエラーを返す

---

## `handleDeleteAiMemory(body)`
...
| プロパティ | 型 | 必須 | 説明 |
|---|---|---|---|
| id | string | 必須 | 更新対象の取引ID |
| category | string | 必須 | 大項目 |
| subcategory | string | 必須 | 中項目 |
| memo | string | 必須 | メモ |
| locked | boolean | - | `categoryLocked`列に設定する値。省略時は`false`（既存動作を維持） |
...
- 更新対象は大項目・中項目・メモ・`categoryLocked`のみ。日付・金額・内容などその他の列は変更しない。`updatedAt` を更新日時で上書きする
- ここで手動編集した内容は、次回同一idのCSVが再取込された際、CSV側の値が空でなければ上書きされる（`handleUpload`参照）。ただし`locked`を`true`にした場合は上書きされない
