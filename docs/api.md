...
- 使用するモデルはスクリプトプロパティ `GEMINI_MODEL`（任意）で固定できる。未設定の場合は無料枠クォータの大きい順に `gemini-2.5-flash` → `gemini-2.5-flash-lite` → `gemini-3-flash` → `gemini-2.0-flash` を試し、クォータ超過（429）時のみ次のモデルにフォールバックする
