...
プロンプトは以下の3セクションを上から順に結合して生成する（属性情報が1件も登録されていない場合はセクション1を省略する）。
...
- `ai_attributes` シートに登録された属性情報（`handleGetAiAttributes`参照）は、上記の「ユーザーの属性・前提条件」セクションとしてプロンプト冒頭に自動で挿入される
...
**注意**
- 未設定の場合、`prompt` はGAS内蔵のデフォルト文言、`model` は空文字列を返す
