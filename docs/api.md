...
- `settings` シートの `notes` キーに現在の取り組み・既に対応済みの事項が設定されている場合、「現在の取り組み・既に対応済みの事項（同じ提案は避けること）」としてプロンプトに追記する（未設定時は追記しない）

---

## `handleGetSettings()`

AIアドバイスのプロンプト・使用モデル・現在の取り組み設定を返す。引数なし。
...
**注意**
- 未設定の場合、`prompt` はGAS内蔵のデフォルト文言、`model`/`notes` は空文字列を返す

---

## `handleUpdateSettings(settings)`

AIアドバイスのプロンプト・使用モデル・現在の取り組み設定を更新する。
...
- `notes` を空文字で保存すると、次回以降AIアドバイスのプロンプトに追記されなくなる
